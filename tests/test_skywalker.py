...
def test_skywalker(RE, lcls_two_bounce_system,
    ...
    goal1 += y1.pix[0]/2
    goal2 += y2.pix[0]/2
    range1 = [y1.pix[0]/2 + x for x in [-500, 500]]
    range2 = [y2.pix[0]/2 + x for x in [-500, 500]]
    ...
    def recover(motor, imager, imager_range):
        def recover_plan():
            ...
            pos1 = [motor.position, imager.cent_x()]
            yield from mv(motor, motor.position + step)
            pos2 = [motor.position, imager.cent_x()]
    ...
    tmo = 5
    if use_recover:
        plan = skywalker([y1, y2], [m1, m2], 'centroid_x', 'alpha',
                         [goal1, goal2], first_steps=step, tolerances=1,
        ...
        plan = skywalker([y1, y2], [m1, m2], 'centroid_x', 'alpha',
                         ...
                         averages=1, timeout=tmo)
    RE(run_wrapper(plan))
    assert np.isclose(y1.cent_x(), goal1, atol=2)
    assert np.isclose(y2.cent_x(), goal2, atol=2)
